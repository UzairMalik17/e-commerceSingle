  ...
  return (
    <div ref={domNode} className="h-[90%] relative w-[270px] self-end">
      <button
        ...
        className="bg-white-off w-full h-full flex justify-between items-center px-4 text-[14px] leading-[1.25rem] text-black-soft font-inter font-semibold rounded-tr-lg rounded-tl-lg"
      >
        <span className="flex justify-center items-center gap-2 capitalize">
          <Icons.Menu
            ClassName="w-[16px] h-[9px] cursor-pointer"
            fill="#1D1D1D"
          />
          All Categories
        </span>
          ...
          {Data.Navbar.AllCategories?.map((item: any, index: number) => {
            return (
              <Link
                ...
                key={index}
                href={item.Route}
                className="w-full h-[40px] px-4 flex justify-start items-center text-[.75rem] leading-[1rem] font-inter font-normal text-black-soft gap-3 border-b-[1px] border-gray"
              >
                {item.Icon}
                {item.Name}
              </Link>
